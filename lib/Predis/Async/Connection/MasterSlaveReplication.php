 ...
 */
class MasterSlaveReplication implements AggregatedConnectionInterface
{
    protected $strategy;
    protected $master;
    protected $slaves;
    ...
    {
        $this->loop = $loop;
        $this->strategy = new ReplicationStrategy();
    }
    ...
    {
        if ($this->loop !== $connection->getEventLoop()) {
            throw new ClientException('The two connections must share the same event loop instance');
        }
    ...
    {
        $this->getConnection($command)->executeCommand($command, $callback);
    }
